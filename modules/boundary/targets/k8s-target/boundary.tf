...
resource "boundary_credential_username_password" "static_db_creds" {
  name                = "static_k8s_db_creds"
  description         = "K8s - DB admin credentials"
...
resource "boundary_target" "eks_postgres_admin" {
  ...
  brokered_credential_source_ids = [
    boundary_credential_username_password.static_db_creds.id //var.boundary_resources.static_db_creds_id
  ]
}
...
resource "boundary_credential_library_vault" "eks_token_readonly" {
  ...
  credential_store_id = var.boundary_resources.vault_credstore_id
...
resource "boundary_role" "db_admin" {
  ...
  principal_ids = [var.boundary_resources.group_admin_id]
}
...
resource "boundary_role" "eks_readonly" {
  ...
  principal_ids = [var.boundary_resources.group_analyst_id]
}
