...
resource "aws_instance" "vault" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ssh -o StrictHostKeyChecking=no -i ${path.root}/generated/${local.key_name} ubuntu@${aws_instance.ingress_worker.public_ip} "scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i /home/ubuntu/ssh_key ubuntu@${self.private_ip}:/home/ubuntu/vault_token /home/ubuntu/vault_token"  
      scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i ${path.root}/generated/${local.key_name} ubuntu@${aws_instance.ingress_worker.public_ip}:/home/ubuntu/vault_token ./generated/
      EOT
  }
  */
