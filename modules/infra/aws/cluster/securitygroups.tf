  ...
  ingress_with_cidr_blocks = [
    {
      ...
      to_port     = 8200
      protocol    = "tcp"
      description = "vault api proxy port"
...
module "controller_internal_lb_sg" {
  source = "terraform-aws-modules/security-group/aws"

  name        = "${var.deployment_id}-controller-internal-api"
  description = "Allow traffic to Controller Internal LB from VPC CIDR"
  vpc_id      = module.vpc.vpc_id
  ...
  ingress_with_cidr_blocks = [
    {
      ...
      protocol    = "tcp"
      cidr_blocks = module.vpc.vpc_cidr_block
      description = "Consul api proxy port"
    }
  ]
  egress_rules       = ["all-all"]
  egress_cidr_blocks = ["0.0.0.0/0"]
